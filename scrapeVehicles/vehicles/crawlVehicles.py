...
def scrape_city_vehicles(session, city):
    page_index = 0
    scraped_vehicles = []
...
def scrapeVehicles(session, cities):
    ...
    for i in range(cities_count):
        new_vehicles = scrape_city_vehicles(session, cities[i])
        ...
        if current_state != cities[i].state:
            ...
            print(
                f"{current_count - prev_count} vehicles scraped for state {current_state}",
                flush=True,
            )
