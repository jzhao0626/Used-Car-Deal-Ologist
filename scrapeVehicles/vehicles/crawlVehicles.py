...
def scrapeVehicles(cities):
    ...
    print('''CREATE TABLE IF NOT EXISTS vehicles(id BIGINT PRIMARY KEY, url TEXT, region TEXT, region_url TEXT, 
    ...
    description TEXT, state TEXT, lat REAL, long REAL, posting_date TEXT)''', flush=True)
    ...
    for city in cities:
        ...
        while not empty:
            print(f"Gathering entries {scrapedInCity} through {scrapedInCity + 120}", flush=True)
            ...
            for item in vehiclesList:
                ...
                print('''INSERT INTO vehicles(id, url, region, region_url, price, year, manufacturer, model, condition,
                ...
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                    (idpk, url, city[2], city[1], price, year, manufacturer, model, condition, cylinders,
                     ...
                     size, vehicle_type, paint_color, image_url, description, lat, long, city[0], posting_date))
                
                scraped += 1
            #these lines will execute every time we grab a new page (after 120 entries)
            print("{} vehicles scraped".format(scraped), flush=True)           